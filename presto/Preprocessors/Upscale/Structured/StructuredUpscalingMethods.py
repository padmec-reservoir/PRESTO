        ...
        for k in xrange(coarse_dims[2]):
            ...
            for j in xrange(coarse_dims[1]):
                for i in xrange(coarse_dims[0]):
                    """
                    fine_elems_in_primal = self.mb.get_entities_by_type(
                        self.primals[(i, j, k)], types.MBHEX)
                    self.mb.tag_get_data(self.gid_tag, fine_elems_in_primal)
                    if self._get_elem_by_ijk((0, j*self.coarse_ratio[1],
                                              k*self.coarse_ratio[2]) ==
                    self.mb.tag_get_data(self.gid_tag, fine_elems_in_primal):
                        print "boundary"

                    if i == 0:
                        self.mb.tag_set_data(self.boundary_tag,
                                             fine_elems_in_primal, 1.0)
                        print fine_elems_in_primal
