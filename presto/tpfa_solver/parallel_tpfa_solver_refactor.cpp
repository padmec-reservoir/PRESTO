...
public:
	...
	TPFASolver (Interface *mb, MPIComm comm, char* perm_tag_name, char *centroid_tag_name, 
    ...
    void run () {
    	...
    	*/
    	...
    	MPIAllreduce(&num_local_elems, &num_global_elems, 1, MPI_INT, MPI_SUM, this->comm);
    	
    	// Get tags on elements and exchange those from shared elements.
    	Tag* tag_handles = this->setup_tags();
    	...
    	Epetra_CrsMatrix A (Copy, row_map, 6);
	    Epetra_Vector b (row_map);
		Epetra_Vector X (row_map);
		
		this->assembly_matrix(&A, &b, volumes, tags_handles);
...
private:
    ...
    Tag* setup_tags () {
    	/*
    		...
    		Returns
    		-------
    		Array containing handles for tags.
    	*/
    ...
    void assembly_matrix (Epetra_CrsMatrix* A, Epetra_Vector* b, Range volumes, Tag* tag_handles) {
    	/*
    		...
    		----------
    		A: Epetra_CrsMatrix*
    			Pointer to an Epetra matrix which stores the coeficients.
    		b: Epetra_Vector*
    			Pointer to an Epetra vector which stores the boundary values.
    		volumes: moab::Range
    	...
    	*/
    	...
    	double c1[3], c2[3], k1[9], k2[9], u[3];
    	...
    	for (Range::iterator it = volumes.begin(); it != volumes.end(); it++) {
    		...
    		if (pressure != 0) {
    			...
    			for (Range::iterator itt = adjacencies.begin(); itt != adjacencies.end(); itt++) {
    				...
    				equiv_perm = this->calculate_equiv_perm(k1, k2, u);
